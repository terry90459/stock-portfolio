...
- **持股資料只存在雲端**，未登入不顯示任何內容，離線時也看不到。
  登入 session 存在 sessionStorage，關掉分頁即失效，每個新分頁都要重新登入。
  localStorage 只留計息切換偏好。
- 需要離線備份時，登入後按帳號列的「匯出 JSON」自行存檔，「匯入 JSON」可還原。
  匯入會比對檔案的 `_exportedAt` 與雲端最新的 `updated_at`，檔案較舊或沒有時間戳時
  會先跳出確認，列出雙方時間與筆數再決定要不要覆蓋。
